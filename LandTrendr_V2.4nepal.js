...
var buildSensorYearCollection = function(year, startDay, endDay, sensor, aoi, exclude){
  ...
  if(startMonth > endMonth){
    var oldYear = (parseInt(year)-1).toString();  //NOTE:  if using dates that span Jan 1, the year recorded is the year of the end of the interval.
    var newYear = year;
...
/*
  LT expects only a single image per year in a time series. There are many ways to
  do best available pixel compositing - we have found that a medoid composite requires little logic
  is robust, and fast
  ...
  if('exclude' in exclude){
    ...
    if('imgIds' in exclude){
      ...
      for(var i=0; i<excludeList.length; i++){
        ...
        collection = collection.filter(ee.Filter.stringContains('system:index', excludeList[i].split('/').slice(-1).toString()).not()); //system:id
...
// Assign the disturbance direction 
// this should be handled with indexFlipper

// var assignDistDir = function(index) {
...
// exports.assignDistDir = assignDistDir; 
...
exports.runLT = function(startYear, endYear, startDay, endDay, aoi, index, ftvList, runParams, maskThese, exclude){
  ...
  // check on the flags.  Neither must be set. 
  maskThese = (typeof maskThese !== 'undefined') ?  maskThese : ['cloud','shadow','snow']; // defaults to mask all three unless set otherwise; added water By Peter C. 5/17/2020
  exclude = (typeof exclude !== 'undefined') ?  exclude : {}; // default to not exclude any images
  
  // First, build the surface reflectance collection of all imagery
  // Included in this step are the masking, exclusion, and medoid compositing. 
  var annualSRcollection = buildSRcollection(startYear, endYear, startDay, endDay, aoi, maskThese, exclude); 
  
  
  // Then build the LT collection.  Calculate the appropriate index for
  // segmentationa, and if necessary add on the collection of images for 
  // the fit-to-vertex step, if desired.  If the ftvList is [], then 
  //   there will be no FTV action. 
  
  var annualLTcollection = buildLTcollection(annualSRcollection, index, ftvList); 
  
  //Attach the image collection to the runParams, as required by the algorithm
...
//Adapting to return a collection of fitted imagery rather than just a single stack

// var getSingleYearFTV = function(lt, year, ftvlist){
    
// }

// Uncomment later
// var getFittedDataCollection = function(lt, startYear, endYear, index, ftvlist){
//   //if(ftvlist === 'undefined'){ftvlist = [index]; print('ftv undefined. ftv switched to index')}
//   var bandNames = getYearBandNames(startYear, endYear);
//   if(typeof ftvlist == 'undefined'){    //if the ftvlist is not passed, use the index from segmentation
//     return lt.select(index.toUpperCase()+'_fit').arrayFlatten([bandNames]);
//   }
  // else {
...
// exports.getFittedDataCollection = getFittedDataCollection;
...
var getSegmentDictionary = function(lt, options) {
  ...
  var dsnr = mag.multiply(100).divide(rmse);                 // make mag relative to fit rmse
...
var runParams = [
  ...
  {label: 'Recovery Threshold:', value: 0.75},
...
exports.bufferPanel = function(){
  ...
  var bufferBox = ui.Textbox({value: 10, style:{stretch: 'horizontal'}});
  return ui.Panel(
    [
      bufferSectionLabel,
